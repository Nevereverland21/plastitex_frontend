    ...
    >
      ...
      >
        ...
        <div className="absolute top-2 left-2 z-10 flex flex-col gap-1 items-start">
          {product.featured && (
            <span className="bg-brand-turquoise text-white text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full shadow-sm">
              Destacado
            </span>
          )}
          {isLowStock && (
            <span className="bg-amber-500 text-white text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full shadow-sm">
              Últimas {product.stock} uds
            </span>
          )}
          {!isOutOfStock && isRetail && product.stock > 5 && (
            <span className="bg-emerald-500 text-white text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full shadow-sm flex items-center gap-1">
        ...
        {product.allows_logo && (
          <div className="absolute top-2 right-2 z-10">
            <span className="bg-white/95 backdrop-blur-sm text-brand-turquoise border border-brand-turquoise/20 text-[9px] font-bold px-1.5 py-0.5 rounded-full flex items-center gap-1 shadow-sm">
        ...
        {isOutOfStock && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/25">
            <span className="bg-rose-600 text-white text-[10px] font-black uppercase tracking-wider px-3 py-1 rounded-full shadow-md">
              Agotado
            </span>
          </div>
        )}
        ...
        {/* Botón hover */}
        <div
          className="absolute inset-x-0 bottom-0 h-10 flex items-center justify-center
                      bg-gradient-to-t from-brand-navy/90 to-brand-navy/50
                      transition-all duration-300 opacity-0 translate-y-2
                      group-hover:opacity-100 group-hover:translate-y-0"
        >
          <span className="text-white text-[11px] font-bold flex items-center gap-1 tracking-wide">
            Ver producto
      ...
      <div className="p-2.5 flex flex-col flex-1 gap-0.5">
        ...
        <div className="mb-0.5">
          <p className="text-[9px] text-brand-turquoise font-bold uppercase tracking-[0.12em] mb-0.5">
            {product.category_name}
          </p>
          <Link href={`/${product.slug}`}>
            <h3 className="text-[13px] font-bold text-brand-navy leading-tight line-clamp-2
        ...
        {/* Tiers / mínimo */}
        <div className="min-h-[22px] flex items-start mt-1">
          {hasTiers ? (
            <div className="flex flex-wrap gap-1">
              {product.pricing_tiers!.slice(0, 2).map((tier, i) => (
                <button
                  ...
                  className={`px-1.5 py-0.5 rounded-full text-[9px] font-bold border transition-all duration-150
                              ${activeTierIndex === i
              ...
              {product.pricing_tiers!.length > 2 && (
                <span className="self-center px-1.5 py-0.5 rounded-full text-[9px] font-bold
            ...
            <div className="flex items-center gap-1 text-amber-600 bg-amber-50 rounded-md px-1.5 py-0.5 w-fit">
              <Users size={9} strokeWidth={2.5} />
              <span className="text-[9px] font-bold">Mín. {product.min_units} uds</span>
            </div>
          ) : null}
        </div>
        ...
        <div className="mt-auto pt-1.5 border-t border-gray-50 flex items-end justify-between gap-2">
          ...
          <div className="text-right shrink-0">
            <p className="text-[9px] text-gray-400 font-semibold mb-0 transition-all duration-150">
              {priceLabel}
            </p>
            <p
              className={`text-sm font-black leading-none transition-all duration-150
